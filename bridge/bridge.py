...
class PrototypeStore:
    ...
    def publish_post(self, payload: dict[str, Any], session: dict[str, str]) -> dict[str, Any]:
        ...
        version = self.db.upsert_survey_version(
            ...
            version_label=version_label or "vA",
            platform=platform,
            caption=caption,
            image_url=image,
            likes_count=parse_int(payload.get("likes"), 0),
            comments_count=parse_int(payload.get("comments"), 0),
            shares_count=parse_int(payload.get("shares"), 0),
            is_default=True,
        )
...
class BasePrototypeHandler(BaseHTTPRequestHandler):
    ...
    def log_message(self, fmt: str, *args: Any) -> None:
        sys.stdout.write(f"[prototype2:{self.server_label}] {self.address_string()} - {fmt % args}\n")
...
(function () {{
  ...
  const publishButton = Array.from(document.querySelectorAll("button")).find(function (button) {{
  ...
  const generateButton = document.getElementById("btn-generate-code");
  ...
  function setUserDetails() {{
    ...
    const logoutRow = Array.from(document.querySelectorAll("#user-dropdown div")).find(function (node) {{
      return node.textContent && node.textContent.includes("Sign out of all accounts");
    }});
    if (logoutRow && !logoutRow.dataset.prototype2Logout) {{
      logoutRow.dataset.prototype2Logout = "true";
  ...
  function currentInviteCode() {{
    if (inviteDisplay && inviteDisplay.textContent.trim()) {{
      return inviteDisplay.textContent.trim().toUpperCase();
    }}

    const generated = randomInviteCode();
    if (inviteDisplay) {{
      inviteDisplay.textContent = generated;
  ...
  function currentImage() {{
    const image = document.querySelector('#preview-container img[alt="News Image"]');
    return image ? image.getAttribute("src") || "" : "";
  }}

  function currentVersion() {{
    const activeTab = document.querySelector(".version-tab.tab-active");
    return activeTab ? activeTab.getAttribute("data-version") || "" : "";
  }}
  ...
  function payloadFromEditor() {{
    return {{
      ...
      platform: document.getElementById("input-platform")?.value || "instagram",
      caption: document.getElementById("input-caption")?.value || "",
      likes: document.getElementById("input-likes")?.value || 0,
      comments: document.getElementById("input-comments")?.value || 0,
  ...
  async function publishCurrentPost(event) {{
    event.preventDefault();
    ...
    publishButton.classList.add("opacity-70", "cursor-not-allowed");
    ...
    try {{
      ...
      if (panel) {{
        panel.innerHTML =
          ...
          "</strong> now contains a " + result.post.platform.toUpperCase() +
          " post.<br>Participant: <a class=\\"underline\\" href=\\"" + result.participantUrl +
          "\\" target=\\"_blank\\">" + result.participantUrl + "</a>";
      }}

      alert("successfully post");
    }} catch (error) {{
      alert("Prototype publish service is not available.");
    }} finally {{
      publishButton.disabled = false;
      publishButton.classList.remove("opacity-70", "cursor-not-allowed");
  ...
  if (publishButton && !publishButton.dataset.prototype2Publish) {{
    publishButton.dataset.prototype2Publish = "true";
    publishButton.addEventListener("click", publishCurrentPost, true);
  }}
}})();
</script>
""".strip()
